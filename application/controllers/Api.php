		...
		if($_SERVER['REQUEST_METHOD'] == 'POST' && trim($_POST["type"]) === "seller"){
			...
			if($otp===$otp_digit){
				...
				if($user['seller_status']==='active'){
					...
					'store_image' => $user['seller_store_image'],
...
function get_subcategories() {
	...
	elseif($_SERVER['REQUEST_METHOD'] == 'POST' && trim($_POST["type"]) === "my"){
		...
		foreach ($subcatg as $row):
			...
			'subcategory_name' => $row['subcategory_name'],
			'subcategory_image' => $row['subcategory_image'],
			'subcategory_status' => $row['product_substatus']
			];
		endforeach;
		$res_data['status'] = 'ok';
		$res_data['subcategories'] = $res_dataa;
...
function update_subcat_status(){
	$id = trim($_POST["id"]);	$type = trim($_POST["type"]); $subid = trim($_POST["subcategory_id"]); $stat = trim($_POST["subcategory_status"]);
	if($type === "b2c"){
		$data = array('product_substatus' => $stat);
		$updated=$this->Api_model->update($data, 'products', array('product_store' => $id, 'product_b2' => '0', 'product_subcategory' => $subid));	
		$rdata['status'] = 'ok';
		$rdata['msg'] = 'B2C Subcategory Online Status';
		$rdata['id'] = $id;
		$rdata['subcategory_id'] = $subid;
	}else{
		$rdata['status'] = 'error';
		$rdata['msg'] = 'Something Wrong!';
	}
...
function upload_product(){
	...
	$stat = $this->Api_model->get_field('product_substatus', 'products', array('product_subcategory' => $_POST['product_subcategory'], 'product_store' => $_POST['id']));
...
function orders(){
	...
	if(!empty($orders)){
	  ...
	  foreach ($orders as $row):
		...
		'order_coupon' => $row['order_coupon'],
			...
			'boy_phone' => $row['boy_phone_code'].$row['boy_phone'],
			'boy_image' => $row['boy_image'],
			'order_assign' => $row['order_assign'],
  ...
  elseif($_SERVER['REQUEST_METHOD'] == 'POST' && trim($_POST["type"]) == "detail"){
	...
	if(!empty($order)){
		$orderItems = $this->Api_model->getOrdersI($order_id, $order['order_status'], $seller_id, $order['user_id']);
...
function assign_order(){
	...
	if($updated===1){
		$this->load->model('Notify_model');
 		$this->Notify_model->boy($order_id, "received"); 
		$res_data['status'] = 'ok';
...
function get_b2b_categories(){
	...
	function get_b2b_subcategories(){
		...
		function product_status(){
			...
			if($type === "main"){
 				$data = array('product_status' => $pstatus);
				$updated=$this->Api_model->update($data, 'products', array('product_store' => $id, 'product_id' => $pid));	
				$rdata['status'] = 'ok';
				$rdata['msg'] = 'Main Product Status Update';
				$rdata['pid'] = $pid;
			}elseif($type === "var"){
				$data = array('vproduct_status' => $pstatus);
				$updated=$this->Api_model->update($data, 'productsvar', array('vproduct_store' => $id, 'vproduct_id' => $pid));	
				$rdata['status'] = 'ok';
				$rdata['msg'] = 'Var Product Status Update';
				$rdata['pid'] = $pid;
			}
			elseif($type === "sub"){
				$data = array('sproduct_status' => $pstatus);
				$updated=$this->Api_model->update($data, 'productssub', array('sproduct_store' => $id, 'sproduct_id' => $pid));	
				$rdata['status'] = 'ok';
				$rdata['msg'] = 'Sub Product Status Update';
				$rdata['pid'] = $pid;
			}
			else{
				$rdata['status'] = 'error';
		...
		function update_online(){
			...
			if($type === "b2c_online"){
				...
				$data = array('seller_ostatus' => $status);
				$updated=$this->Api_model->update($data, 'sellers', array('seller_id' => $id));	
				$rdata['status'] = 'ok';
				$rdata['msg'] = 'B2C Online Status';
				$rdata['id'] = $id;
			}elseif($type === "b2c_get"){
				$id = trim($_POST['id']);
				$stat=$this->Api_model->get_field('seller_ostatus', 'sellers', array('seller_id' => $id));
				$rdata['status'] = 'ok';
				$rdata['seller_ostatus'] = $stat;
				$rdata['msg'] = 'B2C Online Status';
				$rdata['id'] = $id;
			...
			$rdata['seller_store_name']=$this->Api_model->get_field('seller_store_name', 'sellers', array('seller_id' => $id));
