...
function awsupload($file_path, $filename){
 ...
 function getMyProductOnlySub($id, $product_id, $sproduct_id){
    ...
    function getOrdersR($seller_id){
      $this->db->join('users', 'users.user_id = orders.order_user_id');
      $this->db->where('order_seller_id', $seller_id);
    ...
    function getOrdersP($seller_id){
      $this->db->join('users', 'users.user_id = orders.order_user_id');
      $this->db->where('order_seller_id', $seller_id);
      $this->db->where('order_status', '2');
    $this->db->where('order_assign >=', '0');
  ...
  function getOrdersA($seller_id){
      $this->db->join('boys', 'boys.boy_id = orders.order_boy');
      $this->db->join('users', 'users.user_id = orders.order_user_id');
      $this->db->where('order_seller_id', $seller_id);
      $this->db->where('order_assign', '2');
      $this->db->order_by('order_date', 'desc');
    ...
    function getOrdersCn($seller_id){
      $this->db->join('users', 'users.user_id = orders.order_user_id');
      $this->db->where('order_seller_id', $seller_id);
    ...
    function getOrdersD($seller_id, $order_id){
      $this->db->join('users', 'users.user_id = orders.order_user_id');
    ...
    function getOrdersI($order_id, $order_status, $seller_id, $user_id){
...
$name = $row['product_name'].'-'.$query2['vproduct_p1'].'-'.$query2['sproduct_p2'];
          ...
          if($order_status == '3'){ $p_review = $this->getReviewsP($user_id, $row['product_id'], $order_id);
          }else{ $p_review = '0'; }
